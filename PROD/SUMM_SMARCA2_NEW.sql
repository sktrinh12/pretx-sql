...
max(SD_IC50_RAOEC) as SD_IC50_RAOEC,
	...
	max(KH_UM_SM2_SD) as KH_UM_SM2_SD,
...
CASE WHEN summ.cell_line = 'Vero' AND summ.assay_type = 'CellTiter-Glo' AND SUMM.P IS NOT NULL AND SUMM.R IS NOT NULL THEN summ.p
		...
		CASE WHEN T13.ASSAY_TYPE = 'Hibit Kinetics' AND T13.CELL_LINE = 'HeLa-SMARCA2-HiBit' THEN T13.std_kh_um END KH_UM_SM2_SD,
		...
		(SELECT
			...
			POWER(10, AVG(
                CASE 
                    ...
                    ELSE NULL --  handle non-positive values
                END
            )) AS r,
