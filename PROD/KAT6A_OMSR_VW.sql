...
COUNT(formatted_id) OVER (PARTITION BY formatted_id, assay_type,cell_line) as c
                                  ...
                                  WHERE classification <> 'Very Potent' AND
                                      formatted_id IN (SELECT REFERENCE_COMPOUNDS FROM (SELECT SUBSTR(A.REFERENCE_COMPOUNDS,1,10) AS reference_compounds,B.CREATED_DATE,ASSAY_TYPE,
...
COUNT(formatted_id) OVER (PARTITION BY formatted_id, TARGET, COFACTOR, COFACTOR_CONC) as c
                                  ...
                                  WHERE classification <> 'Very Potent' AND
                                      COMPOUND_STATUS IS NULL AND formatted_id IN (SELECT REFERENCE_COMPOUNDS FROM (SELECT A.PROJECT_NAME,SUBSTR(A.REFERENCE_COMPOUNDS,1,10) AS reference_compounds,B.CREATED_DATE,
